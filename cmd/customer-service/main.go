...
func main() {
	...
	cfg := config.Load("customer-service", "8082")
	...
	service := customerservice.NewService(
		customerservice.NewPostgresRepository(db),
		customerservice.NewKafkaPublisher(producer),
	)
	handler := customerservice.NewHandler(service)
	...
	go func() {
		if err := consumer.Run(ctx, service.HandleLeadCreated); err != nil {
			log.Error("lead.created consumer stopped", "error", err)
		}
	}()
	...
	if err := httpx.Run(ctx, log, srv); err != nil {
		log.Error("customer service stopped with error", "error", err)
	}
}
