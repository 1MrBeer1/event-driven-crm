...
func main() {
	...
	cfg := config.Load("lead-service", "8081")
	...
	service := leadservice.NewService(
		leadservice.NewPostgresRepository(db),
		leadservice.NewRedisCache(redisClient, 5*time.Minute),
		leadservice.NewKafkaPublisher(producer),
	)
	handler := leadservice.NewHandler(service)
	...
	if err := httpx.Run(ctx, log, srv); err != nil {
		log.Error("lead service stopped with error", "error", err)
	}
}
